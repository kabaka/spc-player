...
const SPC_MAGIC = new Uint8Array([
  0x53, 0x4E, 0x45, 0x53, 0x2D, 0x53, 0x50, 0x43,
  0x37, 0x30, 0x30, 0x20, 0x53, 0x6F, 0x75, 0x6E,
  0x64, 0x20, 0x46, 0x69, 0x6C, 0x65, 0x20, 0x44,
  0x61, 0x74, 0x61, 0x20, 0x76, 0x30, 0x2E, 0x33,
  0x30,
]);
```
...
function parseTextDate(bytes: Uint8Array): string | null {
  ...
  if (match) {
    return formatDate(parseInt(match[1]), parseInt(match[2]), parseInt(match[3]));
  }

  // Pattern: MM/DD/YYYY or MM-DD-YYYY
  match = str.match(/^(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{4})$/);
  if (match) {
    return formatDate(parseInt(match[3]), parseInt(match[1]), parseInt(match[2]));
  }
...
function parseXid6(data: Uint8Array, offset: number): Xid6Tags | null {
  ...
  // Check magic
  if (data[offset] !== 0x78 || data[offset + 1] !== 0x69 ||
      data[offset + 2] !== 0x64 || data[offset + 3] !== 0x36) {
...
function validateFileSize(file: File): SpcParseError | null {
  if (file.size > SPC_MAX_ACCEPTED_SIZE) {
    return spcParseError('SPC_TOO_LARGE', { fileSize: file.size, maxSize: SPC_MAX_ACCEPTED_SIZE });
  }
  if (file.size < SPC_MIN_PLAYABLE_SIZE) {
    return spcParseError('SPC_TRUNCATED', { fileSize: file.size, minSize: SPC_MIN_PLAYABLE_SIZE });
  }
  return null;
}
```
...
```typescript
function safeSlice(data: Uint8Array, offset: number, length: number): Uint8Array | null {
